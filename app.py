...
with tabs[4]:
    ...
    # Some companies (Apple being the classic example) keep a large share of their
    # liquidity in long-term marketable securities, which is a separate balance-sheet
    # line from "cash + short-term investments." The figure most people mean when they
    # say "Apple's cash pile" includes both — so we show that broader total too,
    # alongside the standard narrower cash figure, rather than picking one silently.
    long_term_investments_row = get_row(balance, [
        "Long Term Investments", "Investments And Advances", "Long Term Equity Investment",
    ])
    lt_investments_val = float(long_term_investments_row.iloc[-1]) if long_term_investments_row is not None else 0
    total_cash_and_investments = (cash_val + lt_investments_val) if cash_val is not None else None
    net_cash_broad = (
        (total_cash_and_investments - debt_val)
        if (total_cash_and_investments is not None and debt_val is not None) else None
    )
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric("Cash & Short-Term Investments", fmt_big(cash_val))
    c2.metric("Total Debt", fmt_big(debt_val))
    c3.metric("Net Cash (short-term basis)", fmt_big(net_cash))

    if lt_investments_val:
        c1b, c2b = st.columns(2)
        c1b.metric("+ Long-Term Investments", fmt_big(lt_investments_val))
        c2b.metric("= Net Cash (incl. long-term investments)", fmt_big(net_cash_broad))
        st.caption(
            "Two net cash figures shown because companies vary in how much liquidity they "
            "park in long-term marketable securities vs. short-term cash — Apple is a well-known "
            "example where the short-term-only figure looks much weaker than the full picture."
        )
